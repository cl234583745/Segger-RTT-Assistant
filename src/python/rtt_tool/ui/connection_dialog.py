...
class _ProbeDetectThread(QThread):
    ...
    def run(self):
        try:
            probes = self._debugger_manager.detect_all_probes(force=True)
...
class ConnectionDialog(QDialog):
    ...
    def _deferred_load(self):
        ...
        try:
            ...
            self._device_list_loaded = True
        try:
            self._load_pyocd_targets_async()
        except Exception as e:
            if self.log_service:
    ...
    def _load_pyocd_targets(self):
        if self._pyocd_targets_loaded:
            return
        from ..runtime.pyocd_target_index import load_index
        entries = load_index()
        if entries:
            target_names = [e.name for e in entries]
            self.pyocd_target_combo.addItems(target_names)
            self._pyocd_target_entries = entries
            self._pyocd_targets_loaded = True
            if self.last_pyocd_target:
                self.pyocd_target_combo.setCurrentText(self.last_pyocd_target)
            if self.log_service:
                self.log_service.debug(f'从pyocd_targets.txt加载 {len(entries)} 个目标')
            return
        self._pyocd_targets_loaded = True

    def _load_pyocd_targets_async(self):
        if self._pyocd_targets_loaded:
            return
        ...
        if entries:
            ...
            if self.last_pyocd_target:
                self.pyocd_target_combo.setCurrentText(self.last_pyocd_target)
            if self.log_service:
                self.log_service.debug(f'从pyocd_targets.txt快速加载 {len(entries)} 个目标')

        self._start_background_refresh()

    def _start_background_refresh(self):
        """后台刷新PyOCD目标索引（从builtin+packs重新生成pyocd_targets.txt）"""
        class _RefreshWorker(QThread):
            finished = pyqtSignal(bool, list)

            def run(self_inner):
                try:
                    from ..runtime.pyocd_target_index import refresh_index
                    entries = refresh_index(self.log_service)
                    self_inner.finished.emit(True, entries)
                except Exception as e:
                    if self.log_service:
                        self.log_service.error(f'后台刷新索引失败: {e}')
                    self_inner.finished.emit(False, [])

        self._bg_refresh_worker = _RefreshWorker()

        def _on_bg_refresh_done(ok, entries):
            if ok and entries:
                target_names = [e.name for e in entries]
                ...
                if self.last_pyocd_target:
                    self.pyocd_target_combo.setCurrentText(self.last_pyocd_target)
                if self.log_service:
                    self.log_service.debug(f'后台刷新完成: {len(entries)} 个目标')
            elif not entries:
                from ..runtime.pyocd_target_index import load_index
                fallback = load_index()
                if fallback and not self._pyocd_target_entries:
                    self.pyocd_target_combo.clear()
                    self.pyocd_target_combo.addItems([e.name for e in fallback])
                    self._pyocd_target_entries = fallback
                    self._pyocd_targets_loaded = True

        self._bg_refresh_worker.finished.connect(_on_bg_refresh_done)
        self._bg_refresh_worker.start()
    ...
    def _on_update_pyocd_targets(self):
        ...
        class _RefreshWorker(QThread):
            ...
            def run(self_inner):
                try:
                    from ..runtime.pyocd_target_index import refresh_index
                    entries = refresh_index(self.log_service, progress_callback=self_inner.progress_msg.emit)
                    self_inner.finished.emit(True, entries)
                except Exception as e:
                    if self.log_service:
                        self.log_service.error(f'刷新索引失败: {e}')
                    self_inner.finished.emit(False, [])
        ...
        self._refresh_worker.progress_msg.connect(lambda msg: progress.setLabelText(msg))
        self._refresh_worker.finished.connect(
            lambda ok, entries: self._on_refresh_finished(ok, entries, user_input_before, progress)
        )
        self._refresh_worker.start()

    def _on_refresh_finished(self, ok, entries, user_input_before, progress):
        progress.close()
        self.pyocd_target_combo.clear()
        self.pyocd_update_btn.setEnabled(True)
        if ok and entries:
            target_names = [e.name for e in entries]
            self.pyocd_target_combo.addItems(target_names)
            self._pyocd_target_entries = entries
            self._pyocd_targets_loaded = True
            if user_input_before:
                self.pyocd_target_combo.setCurrentText(user_input_before)
            if self.log_service:
                self.log_service.success(f'已更新 {len(entries)} 个 PyOCD 目标')
        else:
            if self.log_service:
                self.log_service.warning('更新失败，保留原列表')
    ...
    def _on_pack_download_finished(self, success, msg, progress, target):
        progress.close()
        if success:
            from ..runtime.pyocd_target_index import load_index
            entries = load_index()
    ...
    def _on_browse_device(self):
        ...
        class _LoadDeviceWorker(QThread):
            finished = pyqtSignal(list)

            def __init__(self_inner, device_info_service):
                super().__init__()
                self_inner._dis = device_info_service

            def run(self_inner):
                try:
                    _, info_map = self_inner._dis.load_device_list()
                    self_inner.finished.emit(list(info_map.values()))
                except Exception:
                    self_inner.finished.emit([])

        self._device_load_worker = _LoadDeviceWorker(self._device_info_service)

        def _on_entries_loaded(entries):
            self.browse_btn.setEnabled(True)
            if not entries:
                return
            self._show_device_browse_dialog(entries)

        self._device_load_worker.finished.connect(_on_entries_loaded)
        self._device_load_worker.start()

    def _show_device_browse_dialog(self, entries):
        from PyQt5.QtWidgets import (
            QTableWidget, QTableWidgetItem, QHeaderView, QComboBox,
            QAbstractItemView, QDialogButtonBox,
        )
        ...
        table = QTableWidget()
        cols = ["Name", "Family", "Core", "FlashSize", "RAMSize", "CoreId"]
        ...
        for i, e in enumerate(entries):
            table.setItem(i, 0, QTableWidgetItem(e.name))
            table.setItem(i, 1, QTableWidgetItem(e.family))
            table.setItem(i, 2, QTableWidgetItem(decode_jlink_core(e.core)))
            flash_str = f"{e.flash_size/1024:.0f}KB" if e.flash_size >= 1024 else (str(e.flash_size) if e.flash_size else "")
            ram_str = f"{e.ram_size/1024:.0f}KB" if e.ram_size >= 1024 else (str(e.ram_size) if e.ram_size else "")
            table.setItem(i, 3, QTableWidgetItem(flash_str))
            table.setItem(i, 4, QTableWidgetItem(ram_str))
            coreid_raw = str(e.extra_attrs.get("CoreId", ""))
            table.setItem(i, 5, QTableWidgetItem(decode_jlink_coreid(coreid_raw)))
        layout.addWidget(table)

        def apply_filter():
            text = search_edit.text().lower()
            fam = family_combo.currentText()
            for i in range(table.rowCount()):
                name_item = table.item(i, 0)
                if not name_item:
                    continue
                match_text = not text or any(
        ...
        family_combo.currentTextChanged.connect(apply_filter)
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if row >= 0:
                ...
                if name_item:
                    self.device_combo.setCurrentText(name_item.text())
    
    def _on_browse_pyocd_target(self):
        entries = getattr(self, '_pyocd_target_entries', [])
        if not entries:
            return
        self._show_pyocd_target_browse_dialog(entries)

    def _show_pyocd_target_browse_dialog(self, entries):
        from PyQt5.QtWidgets import (
            QTableWidget, QTableWidgetItem, QHeaderView, QComboBox,
            QAbstractItemView, QDialogButtonBox,
        )
        ...
        for i, e in enumerate(entries):
            table.setItem(i, 0, QTableWidgetItem(e.name))
            table.setItem(i, 1, QTableWidgetItem(e.vendor))
            table.setItem(i, 2, QTableWidgetItem(e.part_number))
            table.setItem(i, 3, QTableWidgetItem(e.families))
        ...
        search_edit.textChanged.connect(apply_filter)
        source_combo.currentTextChanged.connect(apply_filter)
        vendor_combo.currentTextChanged.connect(apply_filter)
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if row >= 0:
                name_item = table.item(row, 0)
                if name_item:
                    self.pyocd_target_combo.setCurrentText(name_item.text())
    ...
    def _on_update_devices(self):
        ...
        class _UpdateDeviceWorker(QThread):
            finished = pyqtSignal(bool, list, str)

            def __init__(self_inner, device_info_service, dll_path):
                super().__init__()
                self_inner._dis = device_info_service
                self_inner._dll = dll_path

            def run(self_inner):
                try:
                    device_names, _ = self_inner._dis.update_device_list(
                        self_inner._dll, progress_callback=None
                    )
                    from ..runtime.path_config import RUNTIME_DEVICES_TXT
                    self_inner.finished.emit(True, device_names or [], RUNTIME_DEVICES_TXT)
                except Exception as e:
                    self_inner.finished.emit(False, [], str(e))

        self._update_device_worker = _UpdateDeviceWorker(self._device_info_service, dll_path)

        def _on_update_finished(ok, device_names, info):
            progress.close()
            self.update_btn.setEnabled(True)
            if ok and device_names:
                self.device_combo.clear()
